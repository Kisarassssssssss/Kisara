...
int16_t base_speed = 1000;  // 基础速度
float Kp = 4.723f;            // 路线偏差系数
float Kd = 8.3434f;            // 阻尼系数
int ks = 20;               //光电管调用函数加权返回参数
...
int16_t output; 
float Kp, Ki, Kd; 
}PIDL={0,0,0,0,0,0,1.38,0.1,0.225},PIDR={0,0,0,0,0,0,1.38,0.1,0.225};
int Lpwm,Rpwm;
...
void LineFollow(void)
{
	...
	dynamic_speed = base_speed * 0.7f; // 刹不住可以加上
	  ...
	  */
    Lpwm = base_speed + turn*0.7;
	  Rpwm = base_speed - turn*0.7;
...
void errortest(void)
{
    ...
    if(scount == 0)
    {
        ...
        else if((HAL_GetTick() - startimec) >= 1200)
        {
            startimec = 0;
